<template>
  <div class="flex flex-col h-full">
    <div v-if="selectedChat && selectedChat.other_person" class="p-4 bg-white shadow-lg mb-1 flex justify-between items-center">
      ...
      <button @click="goToProfile" class="p-2 bg-[#07A984] text-white rounded hover:bg-green-600 transition">House</button>
    </div>
    <div ref="messageList" class="flex-1 p-4 overflow-y-auto flex flex-col-reverse bg-gray-50">
      <div v-for="message in messages" :key="message.id" class="mb-4 flex flex-col">
        <div 
          :class="{
    ...
    <div class="p-4 border-t border-gray-300 bg-white flex items-center">
      <input 
        ...
        class="flex-1 p-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#07A984]"
      />
      <button @click="sendMessage" class="ml-2 pl-3 pr-4 pt-2 pb-2 bg-[#07A984] text-white rounded-full hover:bg-green-600 transition">
  ...
  methods: {
    ...
    async sendMessage() {
      if (this.newMessage.trim() !== '') {
        try {
          const response = await sendMessage(this.newMessage, this.selectedChat.other_person.id);
          if (response.success) {
            const newMsg = {
