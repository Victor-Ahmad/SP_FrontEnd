    ...
    <div class="container mx-auto p-4">
      ...
      <div v-else>
        ...
        <div v-if="images.length && showImages" class="image-grid">
          <div v-for="(image, index) in images" :key="index" class="image relative">
            <img :src="image" class="object-contain rounded-lg p-2" />
            <button @click="removeImage(index)" class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1">
              <i class="fas fa-times"></i>
            </button>
          </div>
        </div>
  
        <button @click="submitForm" class="mt-4 complete-btn text-white p-2 rounded-lg">Complete</button>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    setup() {
      ...
      const submitForm = async () => {
        ...
        try {
          const response = await updateDescriptionAndImages(description.value, images.value);
